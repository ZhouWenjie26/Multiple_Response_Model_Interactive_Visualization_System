...
server <- function(input, output, session) {
  ...
  output$rccc_plot <- renderPlot({
    ...
    plot_subtitle <- if(is_marginal) "Probabilities integrated over γ ~ N(0,1)" else bquote("Conditional on " * gamma[ij] == .(input$gamma))
  ...
  output$info_plot <- renderPlot({
    ...
    plot_title <- if(is_marginal) "Item Information Function — Marginal" else "Item Information Function — Conditional"
    plot_subtitle <- if(is_marginal) "Information integrated over γ ~ N(0,1)" else bquote("Conditional on " * gamma[ij] == .(input$gamma))
